...
def create_script_aware_prompt(premise, hypothesis, language_code):
    ...
    if script == "Arabic":
        ...
        Step 1: Internally transliterate the text into Latin script (do NOT output it).
        Step 2: Reason in {lang}.
        Step 3: Output exactly one word in English:
        ...
        return f"""
        You are a {lang} speaker. Reason in {lang}. Decide the relationship and output exactly one English word: entailment, contradiction, or neutral.
        Premise: "{premise}"
        Hypothesis: "{hypothesis}"
        Answer:""".strip()
...
def run_one_subsample(test_data, indices, model_name):
    ...
    for idx in tqdm(indices, desc="samples", leave=False):
        ...
        for _ in range(2):
            try:
                t0 = time.time()
                resp = ollama.generate(model=model_name, prompt=prompt, options=OLLAMA_OPTIONS)
...
def main():
    ...
    for i in range(N_SUBSAMPLES):
        ...
        print(f" Avg time : {metrics['avg_proc_time_s']:.2f}s")
        print(f" Total time : {duration:.1f}s")
        print(" Predictions:", prediction_summary(df))
        print(" Actual :", Counter(df["true_label"]))
