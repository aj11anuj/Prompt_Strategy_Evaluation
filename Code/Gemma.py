...
class GeminiAfriXNLIEvaluator:
    ...
    def query_gemini(self, model_name, prompt, max_retries=3):
        for attempt in range(max_retries):
            try:
                start_time = time.time()
                response = self.clients[model_name].generate_content(prompt)
                end_time = time.time()
    ...
    def evaluate_strategies(self, num_samples=100, language='swa'):
        ...
        for sample_idx in tqdm(range(num_samples)):
            ...
            for strategy in strategies:
                ...
                for model_name in self.models.keys():
                    result = self.query_gemini(model_name, prompt)
    ...
    def save_results(self, filename="gemini_afrixnli_prompt_strategies.csv"):
        if not self.results:
...
def main():
    API_KEY = ""
    print("=== Prompt Strategies Evaluation on AfriXNLI Dataset using Gemini Models ===")
    evaluator = GeminiAfriXNLIEvaluator(api_key=API_KEY)
    results = evaluator.evaluate_strategies(num_samples=100, language='swa')
    evaluator.save_results("gemini_afrixnli_swa_results.csv")
